				...
				for (int d = 0; d < 16; d++) {
					...
					for (int y = 0; y < 12; y++) {
						...
						for (int x = 0; x < 12; x++) {
							...
							if (i == 17) {
								if (xPix > 3 && xPix < 9 && yPix > 3
										&& yPix < 9) {
									col = 0xff0000 + (t & 1) * 0xff00;
								}
							} else {
								if (t == 1 && xPix > 1 && xPix < 4 && yPix > 3
										&& yPix < 8)
									col = skin;
								if (t == 3 && xPix > 8 && xPix < 11 && yPix > 3
										&& yPix < 8)
									col = skin;

								if (xPix > 1 && xPix < 11 && yPix > 5
										&& yPix < 8) {
									col = clothes;
								}
								if (xPix > 4 && xPix < 8 && yPix > 4
										&& yPix < 8) {
									col = skin;
								}
							}
			...
			winLevel: while (true) {
				...
				while (true) {
					if (gameStarted) {
						...
						// Move player:
						int mouse = m;
						playerDir = Math.atan2(mouse / 240 - 120,
								mouse % 240 - 120);
...

						{
							...
							nextMonster: for (int m = 0; m < 256 + 16; m++) {
								...
								if (monsterData[m * 16 + 11] == 0) {
									...
									if (map[xPos + yPos * 1024] < 0xfffffe
										...
										monsterData[m * 16 + 15] = map[xPos
												+ yPos * 1024];
										map[xPos + yPos * 1024] = 0xfffffe;
										monsterData[m * 16 + 9] = (rushTime > 0 || random
												.nextInt(3) == 0) ? 127 : 0;
								...
								if (monsterData[m * 16 + 10] > 0) {
									...
									for (int i = 0; i < amount; i++) {
										double pow = (random.nextInt(100) * random
												.nextInt(100))
												* poww / 10000 + 4;
										double dir = (random.nextInt(100) - random
												.nextInt(100))
												/ 100.0 * rot;
										double xdd = (Math.cos(playerDir + dir) * pow)
												+ random.nextInt(4)
												- random.nextInt(4);
										double ydd = (Math.sin(playerDir + dir) * pow)
												+ random.nextInt(4)
												- random.nextInt(4);
										int col = (random.nextInt(128) + 120);
										bloodLoop: for (int j = 2; j < pow; j++) {
											int xd = (int) (xPos + xdd * j
													/ pow);
											int yd = (int) (yPos + ydd * j
													/ pow);
											int pp = ((xd) + (yd) * 1024)
													& (1024 * 1024 - 1);
											if (map[pp] >= 0xff0000)
								...
								if (m <= 255) {
									double rx = -(cos * xPlayerDist - sin
											* yPlayerDist);
									double ry = cos * yPlayerDist + sin
											* xPlayerDist;

									if (rx > -6 && rx < 6 && ry > -6 && ry < 6
											&& m > 0) {
										damage++;
										hurtTime += 20;
									}
									if (rx > -32 && rx < 220 && ry > -32
											&& ry < 32
											&& random.nextInt(10) == 0) {
										monsterData[m * 16 + 9]++;
									}
									if (rx > 0 && rx < closestHitDist
											&& ry > -8 && ry < 8) {
									...
									dirLoop: for (int i = 0; i < 2; i++) {
										...
										if (m == 0) {
											...
											double xxd = Math.sqrt(xPlayerDist
													* xPlayerDist);
											double yyd = Math.sqrt(yPlayerDist
													* yPlayerDist);
											if (random.nextInt(1024) / 1024.0 < yyd
													/ xxd) {
												if (yPlayerDist < 0)
													ya--;
												if (yPlayerDist > 0)
											...
											if (random.nextInt(1024) / 1024.0 < xxd
													/ yyd) {
												if (xPlayerDist < 0)
													xa--;
												if (xPlayerDist > 0)
											...
											moved = true;
											double dir = Math.atan2(
													yPlayerDist, xPlayerDist);
											monsterData[m * 16 + 2] = (((int) (dir
										...
										if (xa != 0 || ya != 0) {
											map[xPos + yPos * 1024] = monsterData[m * 16 + 15];
											for (int xx = xPos + xa - 3; xx <= xPos
													+ xa + 3; xx++)
												for (int yy = yPos + ya - 3; yy <= yPos
														+ ya + 3; yy++)
													if (map[xx + yy * 1024] >= 0xfffffe) {
														map[xPos + yPos * 1024] = 0xfffffe;
														monsterData[m * 16 + 8] = random
																.nextInt(25);
														continue dirLoop;
													}
											...
											monsterData[m * 16 + 15] = map[(xPos + xa)
													+ (yPos + ya) * 1024];
											map[(xPos + xa) + (yPos + ya)
													* 1024] = 0xfffffe;
										}
							...
							if (shoot) {
								...
								if (closestHitDist < 120) {
									...
									for (int i = 0; i < 10; i++) {
										double pow = random.nextInt(100)
												* random.nextInt(100) * 8.0
												/ 10000;
										double dir = (random.nextInt(100) - random
												.nextInt(100)) / 100.0;
										int xd = (int) (xx - Math.cos(playerDir
												+ dir)
												* pow)
												+ random.nextInt(4)
												- random.nextInt(4);
										int yd = (int) (yy - Math.sin(playerDir
												+ dir)
												* pow)
												+ random.nextInt(4)
												- random.nextInt(4);
						...
						if (xCam > xWin0 && xCam < xWin1 && yCam > yWin0
								&& yCam < yWin1) {
							continue winLevel;
						}
					}
					...
					for (int y = 0; y < 240; y++) {
						for (int x = 0; x < 240; x++) {
							int noise = random.nextInt(16) * random.nextInt(16)
									/ 16;
							if (!gameStarted)
								noise *= 4;
